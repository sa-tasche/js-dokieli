// dokieli extension content script
// Injected statically as a lightweight sentinel on all pages.
// scripts/dokieli.js is injected lazily by popup.js on user activation.
...
WebExtension.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  try {
    if (request.action === 'dokieli.status') {
      ...
      // scripts/dokieli.js has been injected by the popup before this message arrives.
      ...
      // Config.Button.Info is populated by initButtons() inside DO.U.load().
      // If it's already there, init has already run; otherwise wait for dokieli:ready.
